...
- **パッケージ価格だけでなく「1回あたり」の実コストを必ず算出して明記する**（内容量・入数が商品ごとに違うため、パッケージ価格の並びだけでは比較にならない。2026-08-06、食洗機洗剤の記事でRさんから指摘され発覚）。粉末等で1回の使用量が固定でない場合はメーカー目安を使い、その旨を注記する
- 新記事追加時は `index.html` の記事一覧にも忘れず追記
- **2026-09-18追加：新記事の`<li>`には`data-category`属性でカテゴリを付ける**（トップページのカテゴリ絞り込み機能が参照する）。8つのキーと選び方は`~/.local/config/liner_notes/article-brief.md`「7. index.htmlに追記」節を参照
